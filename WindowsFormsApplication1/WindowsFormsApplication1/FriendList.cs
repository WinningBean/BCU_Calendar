...
namespace WindowsFormsApplication1
{
    public partial class FriendList : Form
    {
        ...
        {
            db.UR_CD = "U100000";
            db.AdapterOpen("select  UR_NM  from USER_TB  WHERE ur_cd  in (select FR_FR_FK from FRIEND_TB where FR_UR_FK = '" + db.UR_CD + "') ORDER BY  UR_NM ASC");
        ...
        private void GroupListBottunClick(object sender , EventArgs e)
        {
            Button mybtn = (Button)sender;
            string str = mybtn.Name;

            for(int i = 0; i < friendTable.Rows.Count; i++)
            {
                DataRow currRow = friendTable.Rows[i];
                if (currRow["FRGR_NM"].ToString().Equals(str))
                {
